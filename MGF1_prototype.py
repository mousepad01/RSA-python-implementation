...
def i2osp(x, l):  # integer to octet string primitive conversion; x -> intregul de convertit, l -> lungimea stringului de returnat

    representation = []

    for i in range(l):

        representation.append(chr(x & 255))
        x >>= 8

    return ''.join(representation[::-1])


def mgf1_sha1(msg, lt):

    to_gen = ''
    h_len = 20  # implementat pe baza sha1
...
def mgf1_sha256(msg, lt):

    to_gen = ''
    h_len = 32  # implementat pe baza sha256
